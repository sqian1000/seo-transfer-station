...
| Codex 扩展 | OpenAI 官方 IDE 扩展，底层与 Codex CLI 共用配置 | `~/.codex/config.toml`、`~/.codex/auth.json`；扩展内也可打开 config | 本站 Codex 手动配置沿用 `https://www.aigcwuji.com/v1` | 不是 Cursor 原生 AI 设置；也不是 Claude Code env |
...
- **Codex 中转地址**：本文示例沿用 `https://www.aigcwuji.com/v1`。
...
4. **不要把 Base URL 的 `/v1` 到处复制**：Claude Code 示例和 Codex、Cline、Roo Code 表单可能不同，按当前工具要求填写。
